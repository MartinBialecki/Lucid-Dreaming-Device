...
//czas startu nadawania sygnałów pomagających w uświadomieniu się we śnie w milisekundach (dodawanie milisekund na podstawie funkcji "millis()")
unsigned long signalsStartTime = 0;
float initialDelayTimeInMinutes = 1; //początkowe opóźnienie nadawania sygnałów w minutach
float additionalDelayTimeInMinutes = 10; //dodatkowe opóźnienie nadawania sygnałów w minutach
...
void setup()
{
  ...
  signalsStartTime = millis() + (initialDelayTimeInMinutes * 60 * 1000);
...
void buttonPressTaskExecutor()
{
  ...
  {
    taskExecuted = true;
    if(signalsStartTime > millis()) {
      signalsStartTime += (additionalDelayTimeInMinutes * 60 * 1000);
    }
    else {
      signalsStartTime = millis() + (additionalDelayTimeInMinutes * 60 * 1000);
    }
    ...
    Serial.println((signalsStartTime - millis())/ 60000);
    SerialBT.print("Czas do rozpoczęcia monitorowania ruchów gałek ocznych w minutach: ");
    SerialBT.println((signalsStartTime - millis())/ 60000);
  }
}
